    ...
    traverse(astResult.ast, {
        ...
        VariableDeclarator(path) {
            if (
                ...
                // named function expression: const variableFunction = function aa() {}
                else if (
                    path.node.init.type === "FunctionExpression" &&
                    path.node.init.id?.type === "Identifier" &&
                path.node.init.body.type === "BlockStatement" &&
                    path.node.init.body.loc
                ) {
                    functionBody = path.node.init.body;
        ...
        // class { method() {} } or class { get property() {} } or class { set property(value) {} } or class { static method() {} } or class { #privateMethod() {} } or class { async method() {} }
        ClassMethod(path) {
            if (
                path.node.body.type === "BlockStatement" &&
                path.node.loc &&
